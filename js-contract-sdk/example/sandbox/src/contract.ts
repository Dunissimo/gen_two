...
export default class PostalServiceContract {
    ...
    async init() {
        ...
        // Predefined users
        const adminAddress = 'USER-admin_admin'
        const rostovEmployee = 'USER-rostov_rostov'
        const taganrogEmployee = 'USER-taganrog_taganrog'
        const regularUser = 'USER-user_user'
    ...
    async register(
        ...
        @Param('homeAddress') homeAddress: string
    ) {
        const key = `USER-${login}_${password}`;
        await preload(this, ['users'])

        if (await this.users.tryGet(key)) {
